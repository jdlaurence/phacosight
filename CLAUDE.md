...
- Data layout (composite corpus): raw datasets live per-dataset under `data/<dataset>/` — `data/cataract1k/{segmentation,phase,full}` and `data/cataracts/` — while derived artifacts (`data/features/`, `data/library/`) stay top-level. Compat symlinks `data/{segmentation,phase,full}` → `cataract1k/…` keep historical run configs working. On the cluster, `data/` is a symlink to `/mnt/data/projects/jd_exp/cataract` — datasets live on that mount, not in the home folder.
- Download Cataract-1K (needs Synapse account + `SYNAPSE_AUTH_TOKEN`): `python scripts/download_data.py --sets segmentation phase` → `data/cataract1k/`.
