...
`runs/pidnet_s_anatomy_instrument/fold*/metrics.json` says `git_sha: d2d3c3c`, but the rerun launched at 13:05:37 with the import fix only committed as `82bc5b5` at 13:05:51 — the run started from a dirty tree and the recorded SHA does not contain the code that ran. Harmless this time (import-path-only fix), but provenance should record a dirty-tree flag or it can't be trusted.
