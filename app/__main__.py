"""Single-command launcher: `python -m app` (or `cataract-insight` once installed).
...
this command is a working local deployment.
    ...
    CATARACT_DATA_ROOT=/mnt/data/... python -m app
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="cataract-insight")
    parser.add_argument("--host", default="127.0.0.1",
                        help="bind address (0.0.0.0 to expose on the network)")
    parser.add_argument("--port", type=int, default=7860)
    parser.add_argument("--data-root", default=None,
                        help="directory holding library/, phase/, full/ "
                             "(default: <repo>/data or $CATARACT_DATA_ROOT)")
    args = parser.parse_args()
    if args.data_root:
        os.environ["CATARACT_DATA_ROOT"] = args.data_root

    import torch
    if torch.cuda.is_available():
        gpus = ", ".join(torch.cuda.get_device_name(i)
                         for i in range(torch.cuda.device_count()))
        print(f"[cataract-insight] inference device: CUDA ({gpus})")
    else:
        print("[cataract-insight] no GPU detected — browsing works normally; "
              "uploaded-video analysis will run on CPU (slow, ~10-20x realtime)")

    import uvicorn
    print(f"[cataract-insight] serving on http://{args.host}:{args.port}")
